...
\begin{figure}
  ...
  \label{DaytodayskillsSelfworth}

<<DaytodayskillsSelfworth, echo=FALSE>>=
figureDaytodayskillsSelfworth = readRDS("./ANALYSIS/GRAPHS/PAPER/Equality_Year.Rds")
...
texreg(list(lm_ts, lm_ts_o,lm_ts_i), caption = "Regression Results: Number of trips", custom.model.names = c("OLS", "OLS without Outliers", "OLS Impute"), scalebox = 0.8, label = 'GrantsRegressionsTrips', digits = 7)
@
...
stargazer(lessIllOdds@coef3, title= "Propodss Regression Results: Less Ill", label = "lessIllOdds")
