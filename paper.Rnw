...
<<Health Regressions Scatterplots, echo=FALSE, results='asis', message=FALSE>>=
Health_plots <- readRDS("./ANALYSIS/GRAPHS/PAPER/Health_plots.Rds")
print(Health_plots)
@

<<Health Regressions percentageplots, echo=FALSE, results='asis', message=FALSE>>=
Health_percentageplots <- readRDS("./ANALYSIS/GRAPHS/PAPER/Health_percentageplots.Rds")
print(Health_percentageplots)
@

<<Health Regressions time plots, echo=FALSE, results='asis', message=FALSE>>=
...
# row.names(tableDispSumm) <- c("$Disp_t^n$", "$Disp_t^d$", "$Disp_t^a$", "$Y_t^n$", "$Y_t^d$", "$GDP_t$")
# colnames(tableDispSumm) <- c("$Corr(X_t, X_{t-1})$", "$\\sigma(X)$", "$\\frac{\\sigma(X)}{\\sigma(Y)}$", "$\\frac{\\sigma(X)}{\\sigma(GDP)}$")
# tableDispSumm <- xtable(tableDispSumm, caption = "Productivity dispersion: Summary statistics", digits = 3, align = "crccc")
# print(tableDispSumm, sanitize.text.function = function(x) {x}, caption.placement = "top")
@
