...
In its yearly surveys CHILDREN asks about three variables closley related to a healthy diet. These are the degree to which beneficiaries: are healthier, have a growing appreciation for a healthier diet, and have increased their knowledge about what constitutes a healthy diet. Figure \ref{Health_timeplots} displays the share of organizarions in each category of the health outcome from 2011 to 2018. The possible values are: all(coded as 4), most (3), some (2), few (1), and none (coded as 0). For this figure, we use the original ordinal varibles which result from the survey structure.  
...
In this section we use a simple linear model with, as described in equation \ref{SimpleLinear Model}. We look at the association between: 
...
@
In the remit of the Trips program, the picture looks much different. 
...
ss = readRDS("./ANALYSIS/Tables/selfworth_scaled.Rds") #standardizedweighted
...
As an option to run an regression on the original ordinal variables, we use the vglm function from the VGAM package... 

Use from zwichenpräsentation
...
stargazer(lessIllOdds@coef3, title= "Propodss Regression Results: Less Ill", align = TRUE)
...
stargazer(dietaryOdds@coef3, title= "Propodss Regression Results: Dietary Knowledge", align = TRUE)
@
<<echo=FALSE, results='asis', message=FALSE>>=

appreciateOdds <- readRDS("./ANALYSIS/Tables/AppreciateProp.Rds")

stargazer(appreciateOdds@coef3, title= "Propodss Regression Results: Appreciate Healthy", align = TRUE)
@
...
stargazer(selfworthOdds@coef3, title= "Propodss Regression Results: Equality of opportunities, selfworth", align = TRUE)
@
...
stargazer(dayToDayOdds@coef3, title= "Propodss Regression Results: Equality of opportunities, everyday expertise", align = TRUE)
@
