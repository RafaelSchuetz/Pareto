...
\begin{equation}
\label{Meals Grants Regression Model}
...
Where: 
y: Health outcomes: lessIll, dietaryKnowledge,  appreciateHealthy
x: DGECriteria Number

<<HealthRegressions, echo=FALSE, results='asis', message=FALSE>>=
>>>>>>> baa67ebc52a230630056f29a4c5002f3940eea0b
...
Where: 
y: Equal Opportunity proxies: selfworth, dayToDaySkills
x: Real Lunch grant, real Trips grant
...
tss = readRDS("./ANALYSIS/Tables/selfworthTrips_scaled.lm.Rds")
...
ssI = readRDS("./ANALYSIS/Tables/selfworth_IM_scaled.Rds") #standardized
